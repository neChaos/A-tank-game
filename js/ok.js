function crash(context,bullet,tank){
    if(context==ctx1&&Math.sqrt(Math.pow(Math.abs(x1-p.x),2)+Math.pow(Math.abs(y1-p.y),2))<=20){
        playDirections[1]=1;
        ...
        for(var i=0;i<=enemys.length-1;i++){
            if(enemys[i].HP==0){
                continue;
            }else if(Math.abs(bullet.x-enemys[i]x+16)<=30&&Math.abs(bullet.y-enemys[i]y+16)<=30){
